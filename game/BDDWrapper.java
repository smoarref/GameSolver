...
//			System.out.println("minterm is "+str);
	...
	public static ArrayList<String> extractMinterms(String[] input){
		ArrayList<String> result=new ArrayList<String>();
		for(String s : input){
			if(s != null) result.addAll(extractMinterms(s));
		}
		return result;
	}
...
//		UtilityMethods.debugBDDMethods(bdd, "in EpreImage, the set is ",  set);
		
		int targetSetPrime=bdd.ref(bdd.replace(set, vTovPrime));
		
//		UtilityMethods.debugBDDMethods(bdd, "target set prime is ", targetSetPrime);
...
//		UtilityMethods.getUserInput();
		...
		bdd.deref(statesWithTransitionToSet);
		bdd.deref(statesWithTransitionsToComplementSet);
...
//		UtilityMethods.getUserInput();
	...
	public static int forall(BDD bdd, int formula, int cube){
		return bdd.ref(bdd.forall(formula, cube));
	}
