...
public class ImplicitGameCaseStudy2 extends Game{
...
//	int[] actions;
	...
	public ImplicitGameCaseStudy2(BDD argBDD){
		super(argBDD);
	}

	
	public ImplicitGameCaseStudy2(BDD argBDD, String argId, int dimension, GridCell2D envInitCell, 
			GridCell2D sysInitCell, Variable[] argEnvX, 
			Variable[] argEnvY, Variable argDirection, Variable[] argSysX, Variable[] argSysY, 
		...
		defineVariables(argEnvX, argEnvY, argDirection,  argSysX, argSysY, argEnvActs, argSysActs);
		...
		init = BDDWrapper.andTo(bdd, init, direction.getBDDVar());
...
//		ArrayList<Integer> T_sysList= singleStepTransition(rx, ry, sameEnvironmentVars, actions);
	...
	private void defineVariables(Variable[] argEnvX, Variable[] argEnvY, Variable argDirection, Variable[] argSysX, Variable[] argSysY, Variable[] argEnvActs, Variable[] argSysActs){
		Variable[] ox = argEnvX;
		Variable[] oy = argEnvY;
		Variable[] rx = argSysX;
		Variable[] ry = argSysY;
		...
		directionPrime = direction.getPrimedCopy();
		...
		Variable[] dirVar = new Variable[]{direction};
		environmentVars = Variable.unionVariables(environmentVars, dirVar);
		
		systemVars= Variable.unionVariables(rx, ry);
		
		environmentVarsPrime=Variable.unionVariables(oxPrime, oyPrime);
		Variable[] dirVarPrime = new Variable[]{directionPrime};
		environmentVarsPrime = Variable.unionVariables(environmentVarsPrime, dirVarPrime);
...
//				int xPrime = BDDWrapper.assign(bdd, envXindex-1, xVarsPrime);
				xState = bdd.andTo(xState, direction.getBDDVar());
				int xPrime;
				int dirPrime;
				if(envXindex == 0 ){
					xPrime = BDDWrapper.assign(bdd, envXindex+1, xVarsPrime);
					dirPrime = bdd.ref(bdd.not(directionPrime.getBDDVar()));
				}else{
					xPrime = BDDWrapper.assign(bdd, envXindex-1, xVarsPrime);
					dirPrime = bdd.ref(directionPrime.getBDDVar());
				}
				
				int trans = bdd.ref(bdd.and(xState,xPrime));
				trans = bdd.andTo(trans, dirPrime);
...
//			if(envXindex < xDimension -1 && !up){
			...
			if(!up){
//				int xPrime = BDDWrapper.assign(bdd, envXindex+1, xVarsPrime);
				xState = bdd.andTo(xState, bdd.not(direction.getBDDVar()));
				int xPrime;
				int dirPrime;
