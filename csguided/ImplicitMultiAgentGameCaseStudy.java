...
public class ImplicitMultiAgentGameCaseStudy extends CompositeGame{
...
//	Variable[] envActionVars;
	...
	public ImplicitMultiAgentGameCaseStudy(BDD argBDD){
		super(argBDD);
	}
	
	public ImplicitMultiAgentGameCaseStudy(BDD argBDD, int dimension, ArrayList<GridCell2D> envInitialCells, ArrayList<GridCell2D> sysInitCells){
		super(argBDD);
...
//				sysInitCells.get(i).print();
				ImplicitGameCaseStudy g = new ImplicitGameCaseStudy(bdd, "game_"+i, dimension, 
								envInitialCells.get(j), sysInitCells.get(i), envXvars.get(j), 
								envYvars.get(j), sysXvars.get(i), sysYvars.get(i), uncontrollabelAgentsActionVars.get(j), 
...
//						UtilityMethods.getUserInput();
	...
	public ImplicitMultiAgentGameCaseStudy(BDD argBDD, int dimension, ArrayList<GridCell2D> initialCells){
		super(argBDD);
		xDimension=dimension;
		yDimension=dimension;
		calculateNumberOfBits();
		
		numOfAgents = initialCells.size();
		numOfControllableAgents = numOfAgents-1;
		
		//define variables and actions
		createVariables(initialCells.size());
//		createVariablesInOrder(initialCells.size());
		...
		for(int i=1; i<numOfAgents; i++){
			ImplicitGameCaseStudy g = new ImplicitGameCaseStudy(bdd, "game_"+i, dimension, 
					initialCells.get(0), initialCells.get(i), envXvars.get(0), 
					envYvars.get(0), sysXvars.get(i-1), sysYvars.get(i-1), envActionVars, 
					agentsActionVars.get(i-1));
			System.out.println("implicit game "+g.id+" was created");
			addGame(g);
		}
		...
		System.out.println("testing composite game's functions");
...
//		 Variable[] oyPrime = new Variable[numOfYBits];
	...
	//TODO: variable ordering, creating actions first or last? 	
	private void createVariables(int numOfAgents){
		
		//creating action vars
		envActionVars = Variable.createVariables(bdd, 2, "act_e");
		 actionVars = envActionVars;
		...
		for(int j=0;j<numOfAgents-1;j++){
		 ...
		 for(int i=0; i<numOfXBits;i++){
			 ox[i]= new Variable(bdd, "OX"+i);
			 oxPrime[i]=Variable.createPrimeVariable(bdd, ox[i]);
			
			 for(int j=0; j<numOfAgents-1; j++){
				 Variable[] agent_j = sysXvars.get(j);
				 agent_j[i]=new Variable(bdd, "RX"+i);
				 Variable[] agent_j_xPrime = sysXvarsPrime.get(j);
				 agent_j_xPrime[i]=Variable.createPrimeVariable(bdd, agent_j[i]);
			 }
		 ...
		 for(int i=0;i<numOfYBits;i++){
			 oy[i]= new Variable(bdd, "OY"+i);
			 oyPrime[i]=Variable.createPrimeVariable(bdd, oy[i]);
			 
			 for(int j=0; j<numOfAgents-1; j++){
				 Variable[] agent_j = sysYvars.get(j);
				 agent_j[i]=new Variable(bdd, "RY"+i);
				 Variable[] agent_j_yPrime = sysYvarsPrime.get(j);
				 agent_j_yPrime[i]=Variable.createPrimeVariable(bdd, agent_j[i]);
			 }
		 }
...
//		
//		for(int i=1;i<numOfAgents;i++){
//			Variable[] rx = Variable.createVariables(bdd, numOfXBits, "RX"+i);
//			Variable[] ry = Variable.createVariables(bdd, numOfYBits, "RY"+i);
...
//			
//			Variable[] agentVars = Variable.unionVariables(rx, ry);
//			Variable[] agentVarsPrime = Variable.unionVariables(rxPrime, ryPrime);
//			
//			if(i==1){
//				systemVars= agentVars;
//				systemVarsPrime=agentVarsPrime;
...
//		
//		for(int i=1;i<numOfAgents;i++){
//			Variable[] rxPrime = Variable.createPrimeVariables(bdd, sysXvars.get(i-1));
//			Variable[] ryPrime = Variable.createPrimeVariables(bdd, sysYvars.get(i-1));
//			sysXvarsPrime.add(rxPrime);
//			sysYvarsPrime.add(ryPrime);
//			Variable[] agentVarsPrime = Variable.unionVariables(rxPrime, ryPrime);
//			if(i==1){
...
//		}
	...
	public void createVariablesInOrder(int numOfAgents){
		//creating action vars
				envActionVars = Variable.createVariables(bdd, 2, "act_e");
				 actionVars = envActionVars;
