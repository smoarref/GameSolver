...
public class FastCSGuidedControl {
...
//	private ArrayList<Integer> betaStatesFocusedSets;
	...
	public FastCSGuidedControl(BDD argBdd , Game game , int[] argAbstractionPredicates, int argObjective){
		bdd=argBdd;
		concreteGame = game;
	...
	public FastCSGuidedControl(BDD argBdd , Game game , AbstractGame initialAbstraction, int argObjective){
		bdd=argBdd;
		concreteGame = game;
...
//		}
		...
		//We deref them during shatterAndRefine -> not anymore
...
//		int eqClassIndex = UtilityMethods.booleanArrayToInteger(abstractStateToBoolArray);
		...
		int[] indices = abstractGame.eqClassIndices(abstractState);
		if(indices.length>1){
			System.err.println("something is wrong in focus, the abstract state is mapped to more than one index!");
		}
		int eqClassIndex = indices[0];
...
//			UtilityMethods.duration(t_avl, "in focus, available actions computed", 500);
			...
			if(availableActions==0){
				envStatesFocusedSets.set(eqClassIndex, abstractGame.concretize(abstractState));	
...
//				UtilityMethods.debugBDDMethods(bdd, "concrete children state", concreteChildrenStates);
//				UtilityMethods.debugBDDMethods(bdd, "cube is ", cube2);
//				UtilityMethods.getUserInput();
