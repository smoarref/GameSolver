...
import testsAndCaseStudies.IntersectionCaseStudy1;
import testsAndCaseStudies.SimpleVehicle;
import testsAndCaseStudies.TestParametricObjectives;
import testsAndCaseStudies.TwoWayRoadCaseStudy;
import utils.UtilityFormulas;
import utils.UtilityMethods;
import csguided.AbstractGame;
import game.BDDWrapper;
import csguided.CaseStudy;
import csguided.CompositeGame;
import csguided.FastCSGuidedControl;
import game.GameSolution;
import game.GameSolver;
import specification.GridCell2D;
import csguided.ImplicitGameCaseStudy;
import csguided.ImplicitGameCaseStudy2;
import csguided.ImplicitMultiAgentGameCaseStudy;
import csguided.ImplicitMultiAgentGameCaseStudy2;
import game.Player;
import csguided.TestFMCAD15;
import game.Variable;
import jdd.bdd.BDD;
import jdd.bdd.Permutation;

public class RoboticMP2DApp extends JFrame {
	 
	enum Approach{
		central,
		compositional,
		csGuidedCentral,
		csGuidedCompositional
	}
		public RoboticMP2DApp(){

	        initUI();
	    }
		
		public RoboticMP2DApp(int xDim, int yDim, BDD bdd, GameStructure argGame, 
	    		ArrayList<Agent2D> agents, ArrayList<GridCell2D> staticObs){
			//visualize the strategy
			int h = 800;
			int w = 800;
			add(new RoboticMP2D(h, w, xDim, yDim, bdd, argGame, agents, staticObs));
		...
		public RoboticMP2DApp(CaseStudy2D caseStudy){
			//visualize the strategy
			int h = 800;
			int w = 800;
			add(new RoboticMP2D(h, w, caseStudy.getXdim()+1, caseStudy.getYdim()+1, caseStudy.getBDD(), 
					caseStudy.getSolution(),caseStudy.getAgents(), caseStudy.getStaticObstacles()));
...
//	    	testCaseStudy();
	    	...
	    	//testImplicitGameCaseStudy();
	    	
//	    	testIntersectionCaseStudy1();
	    	
	    	testParametricObjectives();
			
	    }    
	    
	    
	    public void testIntersectionCaseStudy1(){
	    	int dim=8;
	    	IntersectionCaseStudy1 cs = new IntersectionCaseStudy1(dim-1); 
	    	
	    	//visualize the strategy
			int h = 800;
			int w = 800;
			add(new RoboticMP2D(h, w, dim, dim, cs.getBDD(), cs.getSolution(),cs.getAgents(), cs.getStaticObstacles()));
	    ...
	    public void testParametricObjectives(){
	    	int xDim = 4;
	    	int yDim = 4;
	    	
	    	TestParametricObjectives cs = new TestParametricObjectives(xDim-1);
	    	
	    	//visualize the strategy
			int h = 800;
			int w = 800;
			add(new RoboticMP2D(h, w, xDim, yDim, cs.getBDD(), cs.getSolution(),cs.getAgents(), cs.getStaticObstacles()));
	    ...
	    public void testIntesectionCaseStudy(){
...
//			int obj = BDDWrapper.not(bdd, sameLoc);
			...
			add(new RoboticMP2D(h, w, dim, dim, bdd, sol.strategyOfTheWinner(),agents, staticObstacles));
...
//	    private void testCaseStudy(){
//	    	BDD bdd = new BDD(10000,1000);
//	    	int xDim=4;
//	    	int yDim=16;
//	    	int h = 800;
//			int w = 800;
//			Approach app = Approach.central;
//	    	
...
//	    	
//			int objective = bdd.ref(bdd.and(safetyObjective1, safetyObjective2));
//			objective = bdd.andTo(objective, objective3);
//			
//			ArrayList<Game> concreteGames = new ArrayList<Game>();
//			concreteGames.add(games[0]);
//			concreteGames.add(games[1]);
//			
//			CompositeGame compGame = new CompositeGame(bdd, concreteGames);
//			
//			System.out.println("Games and objectives created");
//			
//			Game solution=null;
//			
//			if(app == Approach.central){
//				solution = TestFMCAD15.testCentral(bdd, concreteGames, objective);
//			}else if(app == Approach.compositional){
//				ArrayList<Integer> individualObjectives = new ArrayList<Integer>();
//				individualObjectives.add(safetyObjective1);
//				individualObjectives.add(safetyObjective2);
//				solution = TestFMCAD15.testCompositional(bdd, concreteGames, individualObjectives, objective3);
//			}else{
////				solution = TestFMCAD15.testCentral(bdd, concreteGames, objective);
//			}
//			
//			TestFMCAD15.checkSolution(bdd, solution, objective, compGame.getVtoVprime());
//			
//			
...
//			sysY.add(cs.R1y);
//			sysY.add(cs.R2y);
//			 add(new RoboticMP2D(h, w, xDim, yDim, bdd, solution, envX, envY, sysX, sysY, null));
//			
//	        setResizable(false);
//	        pack();
//	        
...
//	    }
//	    
//	    private void testImplicitGameCaseStudy(){
//	    	BDD bdd = new BDD(10000,1000);
//			
//			int dim =128;
//			int h = 800;
...
//			}
//			
//			staticObstacles.add(new GridCell2D((dim-1)/2 , 0 ));
//			staticObstacles.add(new GridCell2D((dim-1)/2+1, 0 ));
//			
//			staticObstacles.add(new GridCell2D((dim-1)/2 , dim-1 ));
//			staticObstacles.add(new GridCell2D((dim-1)/2+1, dim-1 ));
//			
...
//			
////			for(int i=0; i<multiAgentGame.getNumberOfUncontrollableAgents();i++){
////				multiAgentGame.noCollisionWithStaticObstacles(envXvars.get(i), envYvars.get(i), staticObstacles);
////				
////			}
//			
////			int pred = BDDWrapper.assign(bdd, dim-2, envYvars.get(0));
////			abstractPreds.add(pred);
//			
//			
...
//			for(int i = 0; i<numOfControllableAgents-1; i++){
...
//////				UtilityMethods.getUserInput();
////				
//////				String namePrefix = "ireach";
//////				Variable[] counterVars = Variable.getVariablesStartingWith(reach.variables, namePrefix);
//////				Variable.printVariables(counterVars);
//////				UtilityMethods.getUserInput();
////				
...
//////					int boundCounter = BDDWrapper.assign(bdd, b, counterVars);
////////					UtilityMethods.debugBDDMethods(bdd, "bound counter ", boundCounter);
//////					abstractPreds.add(boundCounter);
//////					abstractionPredicates.get(i).add(boundCounter);
//////				}
//////				UtilityMethods.getUserInput();
////				
//////				for(int k=1;k<counterVars.length;k++){
//////					int cVar = bdd.ref(counterVars[k].getBDDVar());
//////					abstractPreds.add(cVar);
//////					abstractionPredicates.get(i).add(cVar);
//////				}
////				
...
//////			Variable.printVariables(compGame.primedVariables);
//			
...
////			Game solution = testCentral(bdd, concreteGames, objective);
//			
////			Game solution = testCompositional(bdd, games, noCollisionWithEnvironmentObjectives, collectiveObjective);
//			Game solution = testCompositional(bdd, concreteGames, noCollisionWithEnvironmentObjectives, collectiveObjective);
//			
////			Game  solution = testCSguidedCentral(bdd, multiAgentGame, objective, abstractPreds);
////			Game  solution = testCSguidedCentral(bdd, compGame, objective, abstractPreds);
//			
...
////			Game solution = testCSguidedCompositional2(bdd, compGame, noCollisionWithEnvironmentObjectives, collectiveObjective, abstractionPredicates);
//			
...
//			
//			 add(new RoboticMP2D(h, w, dim, dim, bdd, solution, multiAgentGame.getEnvXvars(), multiAgentGame.getEnvYvars(), multiAgentGame.getSysXvars(), multiAgentGame.getSysYvars(), staticObstacles));
//			
//	        setResizable(false);
//	        pack();
//	        
...
//			
////	    	compositionOfGames.printGameVars();
//////	    	compositionOfGames.printGame();
////	    	UtilityMethods.debugBDDMethods(bdd, "init", compositionOfGames.getInit());
////	    	UtilityMethods.getUserInput();
//			
//			
...
//	    }
//	    
//	    public static Game testCSguidedCompositional(BDD bdd, CompositeGame compGame, ArrayList<Integer> individualObjectives, int collectiveObjective, ArrayList<Integer> abstractPredicates){
...
//	    		FastCSGuidedControl csg = new FastCSGuidedControl(bdd, g, absPreds, individualObjectives.get(i));
//				GameSolution abstractSol = csg.counterStrategyGuidedControl();
//				abstractSol.print();
...
//	    }
//	    
//	    public static Game testCSguidedCompositional2(BDD bdd, CompositeGame compGame, ArrayList<Integer> individualObjectives, int collectiveObjective, ArrayList<ArrayList<Integer>> abstractPredicates){
//	    	System.out.println("cs guided compositional");
//	    	long t0_csguidedComp = UtilityMethods.timeStamp();
...
////	    		UtilityMethods.getUserInput();
////	    		
////	    		Variable[] prime = Variable.getPrimedCopy(compGame.variables);
////	    		Variable.printVariables(prime);
//	    		
//	    		g.setVariables(compGame.variables);
//	    		g.setInit(compGame.getInit());
//	    		
...
//	    }
//	    
//	    public static void testingAbstraction(){
//	    	
//	    }
	    ...
	    public static void main(String[] args) {
	        ...
	        EventQueue.invokeLater(new Runnable() {
	            ...
	            public void run() {
	            	...
	            	if(cs.getAgents() == null) System.out.println("agents are null");
	            	//run the visualization
	               RoboticMP2DApp ex = new RoboticMP2DApp(cs);
	                ex.setVisible(true);
	            }
	        });
	    }
